'''
Script to convert .obj files dumped by NAS-1shot1 to .json for plotting
'''
...
for i in range(n_runs):
    ...
    if algo in ['HB', 'BOHB', 'TPE', 'SMAC']:
        ...
        #print(os.path.join(path, '{}/{}/DE_{}_ssp_{}_seed_{}
        with open(os.path.join(path, '{}/{}/'
...
#     for i in range(n_runs):
#         print("\nRun {}".format(i))
#         with open('experiments/discrete_optimizers/{}/{}/'
#                   'algo_{}_{}_ssp_{}_seed_0.obj'.format(algo, ssp, algo, i, ssp), 'rb') as f:
#             res = pickle.load(f)
#
#         if algo == "TPE":
...
#                 all_runs.append(DotAccess(valid = res[j]['loss'],
#                                           info = res[j].cost,
#                                           test = 1 - res[j].test_accuracy))
#         else:
#             all_runs = res.get_all_runs()
#             all_runs = sorted(all_runs, key=lambda x: x.time_stamps['finished'])
#         incumbents, runtimes = process_and_save(all_runs)
#         with open('experiments/discrete_optimizers/{}/{}/'
#                   'run_{}.json'.format(algo, ssp, i), 'w') as f:
#             json.dump({'runtime': runtimes, 'regret_validation': incumbents}, f)
#
# elif algo in ['RE', 'RS']:
#     for i in range(n_runs):
#         print("\nRun {}".format(i))
#         with open('experiments/discrete_optimizers/{}/{}/'
#                   'algo_{}_0_ssp_{}_seed_{}.obj'.format(algo, ssp, algo, ssp, i), 'rb') as f:
#             res = pickle.load(f)
#
#         all_runs = []
#         for j in range(len(res)):
...
#                                       info = res[j].training_time,
#                                       test = 1 - res[j].test_accuracy))
#         incumbents, runtimes, test_incumbents = process_and_save(all_runs)
#         with open('experiments/discrete_optimizers/{}/{}/'
#                   'run_{}.json'.format(algo, ssp, i), 'w') as f:
#             json.dump({'runtime': runtimes, 'regret_validation': incumbents,
#                        'regret_test': test_incumbents}, f)
#
# elif algo in ['SMAC']:
#     for i in range(n_runs):
...
#         print("\nRun {}".format(i))
#         with open('experiments/discrete_optimizers/{}/{}/'
#                   'DE_{}_ssp_{}_seed_0.obj'.format(i, ssp, i), 'rb') as f:
#             res = pickle.load(f)
#
#         all_runs = []
#         for j in range(len(res)):
#             all_runs.append(DotAccess(loss = 1 - res[j].validation_accuracy,
#                                       info = res[j].training_time,
#                                       test = 1 - res[j].test_accuracy))
#         incumbents, runtimes, test_incumbents = process_and_save(all_runs)
#         with open('experiments/discrete_optimizers/{}/{}/'
